...
import dummyArticles from "./data/articles.ts";

let lastReadImageSrc = "";

export default async (requestInfo: RequestInfo, requestInit?: RequestInit): Promise<Response> => {
...
const dummyFetch = async (path: string, requestInit: RequestInit = { method: "GET" }): Promise<Response> => {
  ...
  if (path === "https://dummyapi.skillerwhale/articles") {
    ...
    if (requestInit.method === "POST") {
      if (!requestInit.body || typeof requestInit.body !== "string") {
        return new Response(`{"error":"bad request: new article request body should be a JSON string"}`);
      }
      try {
        const newArticle = { ...JSON.parse(requestInit.body), id: uuid.v4() };
        if (!isArticle(newArticle)) {
          return new Response(`{"error":"bad request: valid JSON, but not a valid article object"}`);
        }
  ...
  // image
  if (path === "https://dummyapi.skillerwhale/images" && requestInit.method === "POST") {
    if (!requestInit.body) {
      return new Response(`{"error":"bad request: no image data included in request body"}`);
    }
    if (!(requestInit.body instanceof Blob)) {
      return new Response(`{"error":"bad request: image data should be a Blob"}`);
    }
    return new Response(`{"data":{"uri":"${lastReadImageSrc}"}}`);
  }

  // item
  if (path.match(/^https:\/\/dummyapi.skillerwhale\/articles\/articles\/[a-z0-9-]+$/)) {
    // get article
    if (requestInit.method === "GET") {
