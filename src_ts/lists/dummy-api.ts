...
const lotsOfArticles: Article[] = new Array(100)
  .fill(0)
  .map((_, index) => index)
  .map(() =>
...
const response = (offset: number, limit: number) =>
  ({
    json: () =>
...
export const fetchArticles = (offset: number, limit: number) =>
...
export const refreshArticle = (id: string) =>
  new Promise<Article>((resolve) => {
    setTimeout(() => {
